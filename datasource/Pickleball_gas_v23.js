...
//  ✅ v22 FIX: handleAppend dùng đúng liveKey=nd|bang|code (không phải giai|bang|code)
//  ✅ v22 FIX: getDashboardData safety net — KetQua có kết quả → không hiện live dù LiveScore stale
//  ✅ v22 FIX: tournaments filter dùng e.parameter.adminCode (doGet không có data object)
//  ✅ v22 FIX: Manager multi-giai — canAccessTournament check giaiList[], mRows/rRows filter theo tất cả giải
...
const HDR_USERS = ['Mã','Họ tên','Hết hạn','Sân được phân','Vòng đấu','Thiết bị','Ghi chú','Trạng thái','Giải đấu phân','Nội dung phân','Điện thoại','Người tạo'];
// col[11] = ownerCode: Manager tạo TT này — TT thuộc "nhóm" của Manager đó
...
function requireAdminOrManager(data, ss) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (st === 'manager') {
      // giai field (col 8) là comma-separated list mã giải được admin cấp
      const giaiRaw  = String(rows[i][8]||'').trim();
      const giaiList = giaiRaw ? giaiRaw.split(',').map(function(g){return g.trim().toUpperCase();}).filter(Boolean) : [];
      // actor sẽ được build tournMap khi cần (lazy, tránh đọc sheet không cần thiết)
      return { code, name: rows[i][1], role: 'manager', giai: giaiRaw, giaiList, _tournMapBuilt: false };
    }
  }
  return null;
}

// Manager chỉ được thao tác trên giải được phân (hỗ trợ multi-giai, tên lẫn mã)
function canAccessTournament(actor, tournamentId) {
  if (!actor) return false;
  if (actor.role === 'admin') return true;
  const tid = String(tournamentId||'').trim().toUpperCase();
  if (!tid) return false;
  // Dùng expanded list nếu đã build (chứa cả tên lẫn mã)
  if (actor._expandedList && actor._expandedList.length) {
    return actor._expandedList.includes(tid);
  }
  // Fallback: check giaiList
  const list = (actor.giaiList && actor.giaiList.length)
    ? actor.giaiList.map(function(g){return String(g).trim().toUpperCase();})
    : (actor.giai ? [String(actor.giai).trim().toUpperCase()] : []);
  if (list.includes(tid)) return true;
  // Check qua _tournMap nếu có
  if (actor._tournMap) {
    const resolvedId = actor._tournMap[tid];
    if (resolvedId && list.includes(resolvedId)) return true;
  }
  return false;
}

// Build tournament lookup maps cho actor
// Sau khi build: actor._tournMap = { TÊN_HOA: MÃ, MÃ: MÃ }
// Và actor.giaiListExpanded = giaiList + tên giải tương ứng (để canAccessTournament check cả tên lẫn mã)
function _buildTournMapForActor(actor, ss) {
  if (actor._tournMapBuilt) return; // đã build rồi
  actor._tournMapBuilt = true;
  actor._tournMap = {};
  try {
    const sh   = getOrCreateSheet(ss, SH_TOURNAMENTS, HDR_TOURN);
    const tRows = sh.getDataRange().getValues().slice(1);
    tRows.forEach(function(r) {
      const id       = String(r[0]||'').trim().toUpperCase();
      const name     = String(r[1]||'').trim().toUpperCase();
      const ownerCode= String(r[8]||'').trim().toUpperCase();
      if (id) {
        actor._tournMap[id]   = id;   // mã → mã
        if (name) actor._tournMap[name] = id; // tên → mã
      }
      // Nếu manager là owner của giải này → tự động thêm vào giaiList
      if (actor.role === 'manager' && ownerCode && ownerCode === String(actor.code||'').trim().toUpperCase()) {
        if (!actor.giaiList) actor.giaiList = [];
        if (!actor.giaiList.includes(id)) actor.giaiList.push(id);
        if (name && !actor.giaiList.includes(name)) actor.giaiList.push(name);
      }
    });
    // Expand giaiList with tournament names for name-based lookup
    if (actor.giaiList && actor.giaiList.length) {
      const expanded = [];
      actor.giaiList.forEach(function(g) {
        const gUpper = String(g).trim().toUpperCase();
        expanded.push(gUpper);
        // Add resolved id
        const resolvedId = actor._tournMap[gUpper];
        if (resolvedId && !expanded.includes(resolvedId)) expanded.push(resolvedId);
        // Add name for id
        tRows.forEach(function(r) {
          const rid = String(r[0]||'').trim().toUpperCase();
          const rname = String(r[1]||'').trim().toUpperCase();
          if (rid === gUpper || rname === gUpper) {
            if (!expanded.includes(rid)) expanded.push(rid);
            if (rname && !expanded.includes(rname)) expanded.push(rname);
          }
        });
      });
      actor._expandedList = expanded;
    }
  } catch(e) {
    Logger.log('_buildTournMapForActor error: ' + e.message);
  }
}
...
function doGet(e) {
  try {
    ...
    if (action === 'tournaments') {
      const sh   = getOrCreateSheet(ss, SH_TOURNAMENTS, HDR_TOURN);
      const rows = sh.getDataRange().getValues();
      // Lấy actor từ adminCode param (doGet dùng e.parameter, không có data object)
      const adminCodeParam = (e.parameter.adminCode || '').trim().toUpperCase();
      if (adminCodeParam) {
        const uSh2   = getOrCreateSheet(ss, SH_USERS, HDR_USERS);
        const uRows2 = uSh2.getDataRange().getValues();
        for (var ui = 1; ui < uRows2.length; ui++) {
          if (String(uRows2[ui][0]||'').trim().toUpperCase() !== adminCodeParam) continue;
          const uSt = String(uRows2[ui][7]||'').trim().toLowerCase();
          if (uSt === 'manager') {
            // Manager thấy giải:
            //   (A) Giải do chính mình tạo (Tournaments.col[8] = ownerCode)
            //   (B) Giải được admin cấp thêm qua Phân quyền (Users.col[8] = comma-separated giaiIds)
            const giaiRaw2  = String(uRows2[ui][8]||'').trim();
            const assignedIds = giaiRaw2
              ? giaiRaw2.split(',').map(function(g){return g.trim().toUpperCase();}).filter(Boolean)
              : [];
            const filtered2 = rows.filter(function(r, i) {
              if (i === 0) return true; // keep header
              const giaiId    = String(r[0]||'').trim().toUpperCase();
              const ownerCode = String(r[8]||'').trim().toUpperCase(); // Tournaments col 8 = Người tạo
              return ownerCode === adminCodeParam || assignedIds.includes(giaiId);
            });
            return jsonOk({ rows: filtered2 });
          }
          break; // admin hoặc active → trả hết giải
        }
      }
      return jsonOk({ rows });
    }
    ...
    if (action === 'getUsers') {
      ...
      // Manager thấy TT thuộc nhóm của mình:
      // (A) col[11] = ownerCode trùng với mã Manager (TT do Manager này tạo)
      // (B) col[8] (giải phân) là 1 trong các giải Manager quản lý (TT đã được assign vào giải mình)
      const mgrCode   = String(actor.code||'').trim().toUpperCase();
      _buildTournMapForActor(actor, ss);

      // Build tên giải Manager quản lý (cả mã lẫn tên) để match col[8]
      const mgrTournNames = new Set();
      if (actor._expandedList) {
        actor._expandedList.forEach(function(g) { mgrTournNames.add(g.toUpperCase()); });
      } else if (actor.giaiList) {
        actor.giaiList.forEach(function(g) { mgrTournNames.add(String(g).trim().toUpperCase()); });
      }
      // Thêm tên giải resolve từ _tournMap
      if (actor._tournMap) {
        Object.keys(actor._tournMap).forEach(function(k) { mgrTournNames.add(k.toUpperCase()); });
      }

      const out = rows.slice(1).filter(function(r) {
        const st = String(r[7]||'').toLowerCase();
        if (st === 'admin') return false;
        if (st === 'manager') return String(r[0]||'').trim().toUpperCase() === mgrCode; // chỉ thấy bản thân
        // TT (active/khóa): CHỈ thấy TT do chính manager này tạo (ownerCode = mgrCode)
        const ownerCode = String(r[11]||'').trim().toUpperCase();
        return ownerCode === mgrCode;
      });
      return jsonOk({ rows: [rows[0], ...out] });
    }
...
function doPost(e) {
  try {
    ...
    if (action === 'appendResult') {
      ...
      if (!actor) return jsonOk({ ok: false, reason: 'Không có quyền' });
      if (actor.role === 'manager') _buildTournMapForActor(actor, ss);
      ...
      // ── Cập nhật điểm vào sheet Matches (để admin thấy realtime không cần reload) ──
      // row: [time, giai, nd, bang, code, teamA, teamB, scoreA, scoreB, winner, court, ref, rule]
      try {
        const sA    = row[7] !== undefined ? row[7] : '';
        const sB    = row[8] !== undefined ? row[8] : '';
        const mSh   = getOrCreateSheet(ss, SH_MATCHES, HDR_MATCHES);
        const mRows = mSh.getDataRange().getValues();
        const mGiai = String(row[1]||'').trim();
        const mBang = String(row[3]||'').trim();
        const mCode = String(row[4]||'').trim();
        const mRef  = String(row[11]||'').trim();
        for (let i = 1; i < mRows.length; i++) {
          const rGiai = String(mRows[i][0]||'').trim();
          const rBang = String(mRows[i][2]||'').trim();
          const rCode = String(mRows[i][3]||'').trim();
          if ((rGiai === mGiai) && rBang === mBang && rCode === mCode) {
            mSh.getRange(i+1, 6).setValue(sA);   // col F = Điểm A (index 5)
            mSh.getRange(i+1, 7).setValue(sB);   // col G = Điểm B (index 6)
            if (mRef) mSh.getRange(i+1, 11).setValue(mRef); // col K = Ghi chú/TT
            break;
          }
        }
      } catch(e2) { /* không block kết quả nếu update Matches lỗi */ }
    ...
    if (action === 'updateTournament') {
      const actor = requireAdminOrManager(data, ss);
      if (!actor) return jsonOk({ ok: false, reason: 'Không có quyền' });
      // Manager: được sửa giải mình tạo (owner) HOẶC giải được admin cấp quyền
      if (actor.role === 'manager') {
        ...
        const isOwner   = row2 && String(row2[8]||'').trim().toUpperCase() === myCode;
        const isGranted = canAccessTournament(actor, tId);
        if (!isOwner && !isGranted)
          return jsonOk({ ok: false, reason: 'Bạn chỉ được sửa giải của mình hoặc giải được cấp quyền' });
      }
      const sh   = getOrCreateSheet(ss, SH_TOURNAMENTS, HDR_TOURN);
      const rows = sh.getDataRange().getValues();
      const id   = String(data.tournament.id || '').trim().toUpperCase();
    ...
    if (action === 'deleteTournament') {
      const actor = requireAdminOrManager(data, ss);
      if (!actor) return jsonOk({ ok: false, reason: 'Không có quyền' });
      // Manager: chỉ được xóa giải mình tạo (owner) HOẶC giải được cấp quyền
      if (actor.role === 'manager') {
        ...
        const isOwner3   = row3 && String(row3[8]||'').trim().toUpperCase() === myCode3;
        const isGranted3 = canAccessTournament(actor, dId);
        if (!isOwner3 && !isGranted3)
          return jsonOk({ ok: false, reason: 'Bạn chỉ được xóa giải của mình hoặc giải được cấp quyền' });
      }
      const sh   = getOrCreateSheet(ss, SH_TOURNAMENTS, HDR_TOURN);
      const rows = sh.getDataRange().getValues();
    ...
    if (action === 'addUser') {
      ...
      const newPhone = String(data.user.phone||'').trim().replace(/\D/g,'');
      if (!newCode) return jsonOk({ ok: false, reason: 'Thiếu mã' });
      // Mã có thể trùng (không check unique nữa — dùng SĐT để phân biệt)
      // SĐT bắt buộc và phải unique
      if (!newPhone) return jsonOk({ ok: false, reason: 'Thiếu số điện thoại' });
      const dupPhone = rows.slice(1).find(r => String(r[10]||'').trim().replace(/\D/g,'') === newPhone);
      if (dupPhone) return jsonOk({ ok: false, reason: 'Số điện thoại ' + newPhone + ' đã tồn tại (TT: ' + String(dupPhone[1]||'') + ')' });
      const assignGiai = data.user.giai || '';
      // col[11] = ownerCode: Manager tạo TT này → TT thuộc nhóm Manager
      const ownerCode  = actor.role === 'manager' ? String(actor.code||'').trim().toUpperCase() : '';
      ...
      sh.appendRow([
        ...
        String(data.user.phone   || '').trim(),  // col 10 = Điện thoại
        ownerCode                                  // col 11 = Người tạo (ownerCode)
      ]);
    ...
    if (action === 'updateUser') {
      ...
      const sh     = getOrCreateSheet(ss, SH_USERS, HDR_USERS);
      const rows   = sh.getDataRange().getValues();
      const newCode    = String(data.user.code||'').trim();
      const origCode   = data.user.originalCode ? String(data.user.originalCode).trim() : newCode;
      // Kiểm tra SĐT unique (bỏ qua chính row đang sửa)
      const newPhone = data.user.phone !== undefined ? String(data.user.phone||'').trim().replace(/\D/g,'') : null;
      const origPhone = String(data.user.origPhone||'').trim().replace(/\D/g,'');
      if (newPhone !== null) {
        if (!newPhone) return jsonOk({ ok: false, reason: 'Số điện thoại không được để trống' });
        const dupPhone = rows.slice(1).find(r => {
          // bỏ qua chính mình: so theo SĐT cũ hoặc mã gốc
          const rPhone = String(r[10]||'').trim().replace(/\D/g,'');
          const rCode  = String(r[0]||'').trim().toUpperCase();
          const isSelf = (origPhone && rPhone === origPhone) || rCode === origCode.toUpperCase();
          return !isSelf && rPhone === newPhone;
        });
        if (dupPhone) return jsonOk({ ok: false, reason: 'Số điện thoại ' + newPhone + ' đã thuộc TT: ' + String(dupPhone[1]||'') });
      }
      // Tìm row: ưu tiên SĐT cũ, fallback mã gốc
      let foundIdx = -1;
      for (let i = 1; i < rows.length; i++) {
        const rPhone = String(rows[i][10]||'').trim().replace(/\D/g,'');
        const rCode  = String(rows[i][0]||'').trim().toUpperCase();
        if ((origPhone && rPhone === origPhone) || rCode === origCode.toUpperCase()) {
          foundIdx = i; break;
        }
      }
      if (foundIdx < 0) return jsonOk({ ok: false, reason: 'Không tìm thấy' });

      const row = rows[foundIdx];
      // Manager chỉ không được sửa admin hoặc manager khác
      if (actor.role === 'manager') {
        const st = String(row[7]||'').toLowerCase();
        if (st === 'admin') return jsonOk({ ok: false, reason: 'Không có quyền sửa Admin' });
        if (st === 'manager') {
          const isself = String(row[0]||'').trim().toUpperCase() === String(actor.code||'').trim().toUpperCase();
          if (!isself) return jsonOk({ ok: false, reason: 'Không có quyền sửa Manager khác' });
        }
        // TT active/khóa: manager sửa được tất cả TT trong danh sách của mình
      }
      ...
      if (u.giai    !== undefined) sh.getRange(i+1,9).setValue(u.giai);
      if (u.noidung !== undefined) sh.getRange(i+1,10).setValue(u.noidung);
      if (u.phone   !== undefined) sh.getRange(i+1,11).setValue(String(u.phone||'').trim());
      // ownerCode: set nếu chưa có (migration)
      const curOwner = String(rows[i][11]||'').trim();
      if (!curOwner && actor.role === 'manager') {
        sh.getRange(i+1,12).setValue(String(actor.code||'').trim().toUpperCase());
      } else if (u.ownerCode !== undefined && actor.role === 'admin') {
        sh.getRange(i+1,12).setValue(String(u.ownerCode||'').trim().toUpperCase());
      }
      return jsonOk({ ok: true });
    }

    // ── assignManagerGiai: Admin cấp/thu quyền giải cho Manager ──
    // Action riêng biệt, chỉ admin dùng, bypass mọi manager guard
    if (action === 'assignManagerGiai') {
      const adminActor = requireAdmin(data, ss);
      if (!adminActor) return jsonOk({ ok: false, reason: 'Chỉ Admin mới có quyền này' });
      const sh     = getOrCreateSheet(ss, SH_USERS, HDR_USERS);
      const rows   = sh.getDataRange().getValues();
      const target = String(data.mgrCode||'').trim().toUpperCase();
      const giaiStr= String(data.giai||'').trim(); // comma-separated giai IDs
      for (let i = 1; i < rows.length; i++) {
        if (String(rows[i][0]||'').trim().toUpperCase() !== target) continue;
        if (String(rows[i][7]||'').toLowerCase() !== 'manager')
          return jsonOk({ ok: false, reason: 'User không phải Manager' });
        sh.getRange(i+1, 9).setValue(giaiStr); // col I = Giải đấu phân
        return jsonOk({ ok: true, mgrCode: target, giai: giaiStr });
      }
      return jsonOk({ ok: false, reason: 'Không tìm thấy Manager: ' + target });
    }

    if (action === 'deleteUser') {
      const actor = requireAdminOrManager(data, ss);
      if (!actor) return jsonOk({ ok: false, reason: 'Không có quyền [v23]' });
      const sh     = getOrCreateSheet(ss, SH_USERS, HDR_USERS);
      const rows   = sh.getDataRange().getValues();
      const targetPhone = String(data.phone||'').trim().replace(/\D/g,'');
      ...
      // Tìm theo SĐT trước (unique), fallback mã
      let foundIdx = -1;
      for (let i = 1; i < rows.length; i++) {
        const rowPhone = String(rows[i][10]||'').trim().replace(/\D/g,'');
        const rowCode  = String(rows[i][0]||'').trim().toUpperCase();
        if (targetPhone && rowPhone && rowPhone === targetPhone) { foundIdx = i; break; }
        if (!targetPhone && targetCode && rowCode === targetCode) { foundIdx = i; break; }
      }
      if (foundIdx < 0) return jsonOk({ ok: false, reason: 'Không tìm thấy trọng tài [v23]' });

      const row = rows[foundIdx];
      const rowStatus = String(row[7]||'').toLowerCase();

      // Chỉ block 2 trường hợp: xóa admin duy nhất, hoặc manager xóa admin/manager khác
      if (rowStatus === 'admin' && adminCount <= 1)
        return jsonOk({ ok: false, reason: 'Không thể xóa admin duy nhất' });
      if (actor.role === 'manager' && rowStatus === 'admin')
        return jsonOk({ ok: false, reason: 'Manager không thể xóa Admin' });
      if (actor.role === 'manager' && rowStatus === 'manager') {
        const isSelf = String(row[0]||'').trim().toUpperCase() === String(actor.code||'').trim().toUpperCase();
        if (!isSelf) return jsonOk({ ok: false, reason: 'Manager không thể xóa Manager khác' });
      }
      // Tất cả trường hợp còn lại: cho phép xóa
      sh.deleteRow(foundIdx + 1);
      return jsonOk({ ok: true, deleted: String(row[1]||'') });
    }
    ...
    if (action === 'assignRef') {
      ...
      const rows   = sh.getDataRange().getValues();
      const target      = String(data.refCode||'').trim().toUpperCase();
      const targetPhone = String(data.refPhone||data.refCode||'').trim().replace(/\D/g,'');
      for (let i = 1; i < rows.length; i++) {
        const rowCode  = String(rows[i][0]||'').trim().toUpperCase();
        const rowPhone = String(rows[i][10]||'').trim().replace(/\D/g,'');
        const match = (targetPhone && rowPhone && rowPhone === targetPhone) || rowCode === target;
        if (!match) continue;
    ...
    if (action === 'addMatchesBatch') {
      ...
      // Build tournMap: tên giải ↔ mã giải (Manager cần để resolve row[0] = tên giải)
      _buildTournMapForActor(actor, ss);

      const sh   = getOrCreateSheet(ss, SH_MATCHES, HDR_MATCHES);
      const rows = sh.getDataRange().getValues();
      // Build existing key set: GIAI|bang|code (uppercase giai để dedup chính xác)
      const existing = new Set(rows.slice(1).map(r =>
        String(r[0]||'').trim().toUpperCase()+'|'+String(r[2]||'').trim()+'|'+String(r[3]||'').trim()
      ));
      ...
      (data.rows || []).forEach(function(row) {
        const giai = String(row[0]||'').trim();
        if (actor.role !== 'admin' && !canAccessTournament(actor, giai)) {
          blocked.push(giai);
          return;
        }
        // Dedup key dùng uppercase giai để tránh tên/mã mismatch
        const key = giai.toUpperCase()+'|'+String(row[2]||'').trim()+'|'+String(row[3]||'').trim();
        if (!existing.has(key)) toAdd.push(row);
      });

      if (blocked.length > 0 && toAdd.length === 0) {
        return jsonOk({ ok: false, reason: 'Không có quyền tạo lịch cho giải: ' + blocked[0] + '. Kiểm tra phân quyền.' });
      }
      ...
      return jsonOk({ ok: true, added: toAdd.length, skipped: (data.rows||[]).length - toAdd.length, blocked: blocked.length });
    }
    ...
    if (action === 'deleteBang') {
      ...
      if (!actor) return jsonOk({ ok: false, reason: 'Không có quyền' });
      const sh    = getOrCreateSheet(ss, SH_MATCHES, HDR_MATCHES);
      const giai  = String(data.giai  ||'').trim(); // tên giải (giaiName)
      const giaiId= String(data.giaiId||data.giai||'').trim();
      const nd    = String(data.nd    ||'').trim(); // nội dung
      const bang  = String(data.bang  ||'').trim();
      if (!bang) return jsonOk({ ok: false, reason: 'Thiếu bang' });
      ...
      for (let i = rows.length - 1; i >= 1; i--) {
        ...
        const rBang = String(rows[i][2]||'').trim();
        // Match giải: so sánh cả tên giải lẫn mã giải
        const giaiMatch = !giai || rGiai === giai || rGiai === giaiId;
        // Match nội dung (nếu có)
        const ndMatch = !nd || rNd === nd;
        if (giaiMatch && ndMatch && rBang === bang) {
          toDelete.push(i + 1); // 1-based sheet row
        }
      }
      if (toDelete.length === 0) return jsonOk({ ok: true, deleted: 0, note: 'Không tìm thấy trận nào' });
      // Xóa từ dưới lên (toDelete đã sort ngược)
...
function handleValidate(e, ss, tz) {
  ...
  const phone  = (e.parameter.phone  || '').trim().replace(/\D/g,''); // chỉ giữ số
  const device = (e.parameter.device || '').trim();
  const timeStr = Utilities.formatDate(new Date(), tz, 'dd/MM/yyyy HH:mm:ss');

  // Phải có ít nhất mã hoặc SĐT
  if (!code && !phone) return jsonOk({ valid: false, reason: 'Thiếu thông tin đăng nhập' });

  const sh    = getOrCreateSheet(ss, SH_USERS,    HDR_USERS);
  const logSh = getOrCreateSheet(ss, SH_LOGINLOG, HDR_LOGINLOG);
  const rows  = sh.getDataRange().getValues();

  // ── Tìm user: ưu tiên SĐT (col 10), fallback mã (col 0) ──
  let matched = [];
  if (phone) {
    // Đăng nhập bằng SĐT — tìm tất cả user có SĐT khớp
    for (let i = 1; i < rows.length; i++) {
      const rowPhone = String(rows[i][10]||'').trim().replace(/\D/g,'');
      if (rowPhone && rowPhone === phone) matched.push({ rowIdx: i, row: rows[i] });
    }
    if (matched.length === 0) {
      // Thử tìm theo mã nếu SĐT không có kết quả (backward compat)
      if (code) {
        for (let i = 1; i < rows.length; i++) {
          if (String(rows[i][0]||'').trim().toUpperCase() === code) matched.push({ rowIdx: i, row: rows[i] });
        }
      }
      if (matched.length === 0) {
        logSh.appendRow([timeStr, phone||code, '–', device, '❌ Sai SĐT', 'Số điện thoại không tồn tại', '', '']);
        return jsonOk({ valid: false, reason: 'Số điện thoại không tồn tại trong hệ thống' });
      }
    }
    // SĐT nên là duy nhất — nếu trùng, log cảnh báo nhưng vẫn dùng cái đầu tiên
    if (matched.length > 1) {
      Logger.log('WARN: Trùng SĐT ' + phone + ' — ' + matched.length + ' tài khoản');
    }
  } else {
    // Đăng nhập bằng mã (legacy / admin)
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][0]||'').trim().toUpperCase() === code) matched.push({ rowIdx: i, row: rows[i] });
    }
    if (matched.length === 0) {
    ...
    if (matched.length > 1) {
      // Mã trùng mà không có SĐT → yêu cầu SĐT
      return jsonOk({ valid: false, reason: 'duplicate_code', needPhone: true,
        message: 'Mã đăng nhập bị trùng. Vui lòng nhập số điện thoại để xác định tài khoản.' });
    }
  }

  // ── Xử lý user duy nhất ──
  const { rowIdx: i, row: userRow } = matched[0];
  const name      = String(userRow[1]||'').trim();
  const expireRaw = userRow[2];
  const courts    = String(userRow[3]||'').trim();
  const rounds    = String(userRow[4]||'').trim();
  const statusRaw = userRow[7];
  const status    = String(statusRaw||'').trim().toLowerCase();
  const giai      = String(userRow[8]||'').trim();
  const noidung   = String(userRow[9]||'').trim();
  const loginId   = phone || code; // dùng SĐT hoặc mã để log

  if (status === 'khóa' || status === 'disabled') {
    logSh.appendRow([timeStr, loginId, name, device, '❌ Bị khóa', 'Tài khoản bị khóa', courts, rounds]);
    return jsonOk({ valid: false, reason: 'Tài khoản bị khóa', name });
  }

  if (!expireRaw) {
    logSh.appendRow([timeStr, loginId, name, device, '⚠️ Hết hạn', 'Không có ngày hết hạn', courts, rounds]);
  ...
  if (expDate < today) {
    ...
    logSh.appendRow([timeStr, loginId, name, device, '⚠️ Hết hạn', 'Hết hạn: ' + exp, courts, rounds]);
    return jsonOk({ valid: false, reason: 'Tài khoản hết hạn ngày ' + exp, name });
  }

  if (device && !String(userRow[5]||'').trim()) sh.getRange(i+1, 6).setValue(device);
  ...
  logSh.appendRow([timeStr, loginId, name, device, '✅ Đăng nhập',
    expFmt + (isAdmin?' [ADMIN]':isManager?' [MANAGER]':'') + (phone?' [SĐT]':' [Mã]'),
    courtList.join(',') || '—', roundList.join(',') || '—',
    giai || '—', noidung || '—'
  ]);

  return jsonOk({
    valid: true, name, code: String(userRow[0]||'').trim(), expire: expFmt,
    courts: courtList, rounds: roundList, giai, noidung,
    assignment, isAdmin, isManager,
...
function handleAppend(data, ss, tz) {
  ...
  const key        = giai+'|'+bang+'|'+matchCode;  // key cho KetQua upsert
  const liveKey    = nd+'|'+bang+'|'+matchCode;    // key cho LiveScore (noidung|bang|code)
  ...
  // Xóa khỏi LiveScore — dùng liveKey (noidung|bang|code) vì đó là format LiveScore dùng
  clearLive(ss, liveKey);
  // Fallback: cũng thử xóa với key cũ (giai|bang|code) phòng trường hợp data cũ
  if (liveKey !== key) clearLive(ss, key);
...
function makeFilter(params) {
  ...
  const refName = (params.refName || params.refCode || '').trim().toLowerCase();
  return (r) => {
    if (giai  && String(r[0]||'') !== giai)  return false;
    if (nd    && String(r[1]||'') !== nd)    return false;
    if (bang  && String(r[2]||'') !== bang)  return false;
    if (court && !String(r[8]||'').toLowerCase().includes(court.toLowerCase())) return false;
    if (round && !String(r[9]||'').toLowerCase().includes(round.toLowerCase())) return false;
    // ── Lọc theo tên trọng tài (col 10 = GhiChú/referee) ──
    // Col 10 có thể chứa nhiều TT: "CƯƠNG,THANH NGUYET"
    if (refName) {
      const matchRef = String(r[10]||'').trim().toLowerCase();
      if (!matchRef) return false;
      const refNames = matchRef.split(',').map(s=>s.trim()).filter(Boolean);
      const ok = refNames.some(rn => rn === refName || rn.includes(refName) || refName.includes(rn));
      if (!ok) return false;
    }
    return true;
  };
...
function handleDashboard(e, ss, actor, tz) {
  ...
  // Manager: filterGiai = param nếu có (drill-down), ngược lại để trống → getDashboard tự filter theo giaiList
  const rawFilterGiai = actor.role === 'manager'
    ? (giaiParam || '')  // empty = show all assigned giais (filtered below)
    : giaiParam;
  // For manager with no param: filter mRows/rRows by giaiList
  const managerGiaiList = (actor.role === 'manager' && actor.giaiList && actor.giaiList.length)
    ? actor.giaiList : null;
  ...
  // Manager với multi-giai: resolve tên tất cả giải được phân
  const managerGiaiNames = managerGiaiList
    ? managerGiaiList.map(function(gid) { return resolveGiaiName(gid) || gid; }).filter(Boolean)
    : null;

  const allMRows = mSh.getDataRange().getValues().slice(1);
  const mRows = allMRows.filter(function(r) {
    const rGiai = String(r[0]||'').trim();
    if (filterGiai  && rGiai !== filterGiai) return false;  // drill-down filter
    if (!filterGiai && managerGiaiNames && !managerGiaiNames.includes(rGiai)) return false; // manager multi-giai
    if (nd   && String(r[1]||'').trim() !== nd)   return false;
    if (bang && String(r[2]||'').trim() !== bang)  return false;
    return true;
  });
  ...
  const rRows = rSh.getDataRange().getValues().slice(1).filter(function(r) {
    const rGiai = String(r[1]||'').trim();
    if (filterGiai  && rGiai !== filterGiai) return false;
    if (!filterGiai && managerGiaiNames && !managerGiaiNames.includes(rGiai)) return false;
    if (nd   && String(r[2]||'').trim() !== nd)   return false;
    if (bang && String(r[3]||'').trim() !== bang)  return false;
    return true;
  });
  // ══════════════════════════════════════════════════════
  ...
  // Build Set kết quả đã có trong KetQua: nd|bang|code → để loại bỏ false-positive "playing"
  // Safety net: nếu KetQua đã có kết quả mà LiveScore vẫn 'playing' (finishLive fail) → không coi là live
  const doneNdBangCode = new Set();
  rRows.forEach(function(r) {
    const nd2 = String(r[2]||'').trim(), bg2 = String(r[3]||'').trim(), cd2 = String(r[4]||'').trim();
    if (nd2 && bg2 && cd2) doneNdBangCode.add(nd2+'|'+bg2+'|'+cd2);
  });
  ...
  allLiveRawRows.forEach(function(r) {
    ...
    // Nếu đã có KetQua → LiveScore đã stale (finishLive bị miss) → bỏ qua, tự dọn
    if (lKey && doneNdBangCode.has(lKey)) {
      // Dọn row stale này bất đồng bộ (fire-and-forget)
      try {
        const lSh2 = ss.getSheetByName(SH_LIVE);
        if (lSh2) {
          const lastR = lSh2.getLastRow();
          if (lastR > 1) {
            const kc = lSh2.getRange(2, 1, lastR-1, 1).getValues();
            for (var ci = kc.length-1; ci >= 0; ci--) {
              if (String(kc[ci][0]||'').trim() === lKey && String(allLiveRawRows[ci] ? allLiveRawRows[ci][9]||'' : '') === 'playing') {
                lSh2.deleteRow(ci+2); break;
              }
            }
          }
        }
      } catch(ec) {}
      return; // không thêm vào playingKeys
    }
  ...
  const lRows = allLiveRawRows.filter(function(r) {
    ...
    const lKey2 = String(r[0]||'');
    if (!lKey2) return false;
    if (doneNdBangCode.has(lKey2)) return false; // đã có KetQua → không hiện live
    if (filterGiai) {
      const g = String(r[1]||'') || matchKeyToGiai[lKey2] || '';
  ...
  const tournaments = tRows.map(r => ({
    ...
    owner: String(r[8]||'') // col[8] = Người tạo (ownerCode)
  }));
