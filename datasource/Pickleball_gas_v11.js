// ═══════════════════════════════════════════════════════════════
//  PICKLEBALL REFEREE – Google Apps Script v8 (2026-05-21)
...
function handleDashboard(e, ss, actor, tz) {
  ...
  // doneBangCode: Set "bang|code" (không kể giải) để loại live đã xong
  ...
  const lRows = lSh.getDataRange().getValues().slice(1).filter(function(r) {
    ...
    if (parts.length >= 3) {
      ...
      if (doneBangCode[bangCode]) return false;
    }
    return true;
  });
  ...
  // Live detail
  const liveDetail = lRows.map(r => ({
    key: r[0], giai: r[1], teamA: r[2], scoreA: r[3],
    scoreB: r[4], teamB: r[5], serve: r[6], game: r[7], updated: r[8]
  }));
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][0]||'').trim() === giai &&
          String(rows[i][1]||'').trim() === nd   &&
          String(rows[i][2]||'').trim() === bang  &&
          String(rows[i][3]||'').trim() === matchCode) {
