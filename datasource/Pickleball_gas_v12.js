// ═══════════════════════════════════════════════════════════════
//  PICKLEBALL REFEREE – Google Apps Script v12 (2026-05-22)
//  ✅ Fix: trận đánh lại → hiển thị 'Đang thi đấu' đúng trên Admin
...
const HDR_TOURN = ['Mã giải','Tên giải','Địa điểm','Cụm sân','Ngày bắt đầu','Ngày kết thúc','Trạng thái','Ghi chú'];
...
const HDR_LIVE = ['Key','Giải đấu','Đội A','Điểm A','Điểm B','Đội B','Giao bóng','Game','Cập nhật lúc','Trạng thái'];
...
function doGet(e) {
  try {
    ...
    if (action === 'tournaments') {
      ...
      return jsonOk({ rows: sh.getDataRange().getValues() });
    }
...
function doPost(e) {
  try {
    ...
    if (action === 'live') {
      ...
      lock.waitLock(3000); // giảm từ 5s → 3s
      try {
        const sh  = getOrCreateSheet(ss, SH_LIVE, HDR_LIVE);
        ...
        // ── TỐI ƯU 1: Referee gửi kèm giai trong payload → KHÔNG đọc Matches sheet ──
        // Nếu không có → fallback tra Matches (tương thích ngược với referee cũ)
        ...
        const rowWithTs = [
          key, giaiFromMatch,
          r[1]||'', r[2]||0, r[3]||0, r[4]||'', r[5]||'', r[6]||'Game 1', now, 'playing'
        ];

        // ── TỐI ƯU 2: Cache row index trong PropertiesService → KHÔNG đọc toàn bộ sheet ──
        ...
        if (cachedRow > 1) {
          // Thử ghi thẳng vào row đã cache — kiểm tra key khớp trước
          try {
          ...
          } catch(eCache) { /* cache stale, sẽ scan lại */ }
        }

        if (!rowWritten) {
          // Scan tìm row (chỉ đọc cột Key = cột 1, không đọc toàn bộ sheet)
          const lastRow = sh.getLastRow();
          let foundRow = 0;
          if (lastRow > 1) {
            ...
            for (let i = keyCol.length - 1; i >= 0; i--) {
              if (String(keyCol[i][0]||'').trim() === key.trim()) {
                foundRow = i + 2; // 1-based + header offset
                break;
              }
            }
          }
          if (foundRow > 0) {
            ...
            props.setProperty(cacheKey, String(foundRow)); // cập nhật cache
          } else {
            sh.appendRow(rowWithTs);
            // Lưu row mới vào cache
        ...
        if (cachedRow2 > 1) {
          try {
            ...
            if (String(existing2||'').trim() === key.trim()) {
              ...
              sh.getRange(cachedRow2, 1, 1, 10).setValues([[r2[0],r2[1],r2[2],sA,sB,r2[5],r2[6],r2[7],now2,'finished']]);
              found = true;
            }
          } catch(e2) {}
        }
          ...
          if (lastRow2 > 1) {
            ...
            for (let i = 0; i < keyCol2.length; i++) {
              if (String(keyCol2[i][0]||'').trim() === key.trim()) {
                ...
                const sA = (data.scoreA !== undefined && data.scoreA !== null) ? data.scoreA : r2[3];
                const sB = (data.scoreB !== undefined && data.scoreB !== null) ? data.scoreB : r2[4];
                sh.getRange(ri, 1, 1, 10).setValues([[r2[0],r2[1],r2[2],sA,sB,r2[5],r2[6],r2[7],now2,'finished']]);
                found = true;
                break;
              }
            }
    ...
    if (action === 'addTournament') {
      const actor = requireAdmin(data, ss);
      if (!actor) return jsonOk({ ok: false, reason: 'Chỉ admin mới tạo giải' });
      const sh   = getOrCreateSheet(ss, SH_TOURNAMENTS, HDR_TOURN);
      const rows = sh.getDataRange().getValues();
      ...
      sh.appendRow([id, t.name||'', t.venue||'', t.courts||'', t.startDate||'', t.endDate||'', t.status||'active', t.note||'']);
    ...
    if (action === 'updateTournament') {
      const actor = requireAdmin(data, ss);
      if (!actor) return jsonOk({ ok: false, reason: 'Chỉ admin mới sửa giải' });
      const sh   = getOrCreateSheet(ss, SH_TOURNAMENTS, HDR_TOURN);
      const rows = sh.getDataRange().getValues();
      const id   = String(data.tournament.id || '').trim().toUpperCase();
      for (let i = 1; i < rows.length; i++) {
    ...
    if (action === 'deleteTournament') {
      const actor = requireAdmin(data, ss);
      if (!actor) return jsonOk({ ok: false, reason: 'Chỉ admin mới xóa giải' });
      const sh   = getOrCreateSheet(ss, SH_TOURNAMENTS, HDR_TOURN);
      const rows = sh.getDataRange().getValues();
    ...
    if (action === 'assignCourtToBang') {
      ...
      for (let i = 1; i < rows.length; i++) {
        if (String(rows[i][0]||'').trim() === giai &&
          ...
          sh.getRange(i+1, 9).setValue(court); // cột Sân (index 8 → cột 9)
          updated++;
        }
      }
...
function handleDashboard(e, ss, actor, tz) {
  ...
  // doneBangCode: Map "bang|code" → true để loại live đã xong
  // Dùng bang|code thay vì giai|bang|code để tránh mismatch tên/mã giải
  const doneBangCode = {};
  rRows.forEach(function(r) {
    const bg = String(r[3]||'').trim(), cd = String(r[4]||'').trim();
    if (bg && cd) doneBangCode[bg + '|' + cd] = true;
  });

  // ── FIX: Build set các bangCode đang có live 'playing' ──
  // Nếu trọng tài đánh lại → LiveScore có 'playing', dù KetQua đã có bản cũ
  // → ưu tiên live 'playing', không lọc bỏ theo doneBangCode
  const allLiveRawRows = lSh.getDataRange().getValues().slice(1);
  const playingBangCode = new Set();
  allLiveRawRows.forEach(function(r) {
    const lKey  = String(r[0]||'');
    const status = String(r[9]||'');
    if (status === 'finished' || !lKey) return;
    // key format: "noidung|bang|code" → parts[1]|parts[2] = bangCode
    const parts = lKey.split('|');
    if (parts.length >= 3) {
      playingBangCode.add(parts.slice(1).join('|'));
    }
  });

  const lRows = allLiveRawRows.filter(function(r) {
    const lKey = String(r[0]||'');
    // Loại bỏ nếu trọng tài đã đánh dấu hoàn thành (status = 'finished')
    const status = String(r[9]||'');
    if (status === 'finished') return false;
    // Lọc theo giải
    if (filterGiai) {
      const g = String(r[1]||'') || matchKeyToGiai[lKey] || '';
      if (g !== filterGiai) return false;
    }
    // Loại bỏ live nếu trận đã có trong KetQua (double-check)
    // NHƯNG: nếu đang có 'playing' cho bangCode này → trọng tài đánh lại → KHÔNG lọc
    const parts = lKey.split('|');
    if (parts.length >= 3) {
      const bangCode = parts.slice(1).join('|');
      if (doneBangCode[bangCode] && !playingBangCode.has(bangCode)) return false;
    }
    return true;
  });
  ...
  const doneMatches    = rRows.length;
  ...
  mRows.forEach(r => {
    ...
    if (doneBangCode[bc]) tree[g].noidungs[nd].bangs[bg].done++;     // dùng bang|code
    if (liveKeys.has(lk)) tree[g].noidungs[nd].bangs[bg].live++;     // dùng noidung|bang|code
  });
